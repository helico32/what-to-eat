...
function EditForm({ recipe, onSave, onCancel }) {
  ...
  return (
    <div className="flex-1 px-5 pt-4 pb-10">
      {/* Emoji */}
      <div className="mb-5">
        <label className="font-body font-semibold text-[16px] text-ink-secondary mb-2 block">Emoji</label>
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={() => name.trim() && onSave({ emoji, name: cap(name.trim()), time: time.trim() || '15 min', ingredients, steps })}
          disabled={!name.trim()}
