 import { useNavigate, useLocation } from 'react-router-dom'
import { btnActive, btnDefault } from '../utils/styles'

function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
    </svg>
  )
}

function UserIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/>
    </svg>
  )
}

function CartIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
      <path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/>
    </svg>
  )
}

const NAV = [
  { label: 'Dashboard',        path: '/'         },
  { label: 'Planning repas',   path: '/planning' },
  { label: 'Liste de courses', path: '/list'     },
  { label: 'Recettes',         path: '/recipes'  },
]

export default function DesktopHeader({ onDashboard, isAnonymous, authLoading, onShowPlan, onShowAccount, onCart, cartCount }) {
  const navigate = useNavigate()
  const location = useLocation()

  return (
    <header className="sticky top-0 z-20 bg-canvas/90 backdrop-blur-md border-b border-ink-primary px-8">
      <div className="max-w-[1440px] mx-auto flex items-center h-20 gap-6">

        {/* Logo */}
        <button
          onClick={() => { navigate('/'); onDashboard?.() }}
          className="font-display font-bold text-[20px] text-ink-primary flex-shrink-0 hover:opacity-75 transition-opacity"
        >
          What to eat
        </button>

        {/* Centre : liens nav + bouton + */}
        <nav className="flex items-center gap-3 flex-1 justify-center">

          {NAV.map(({ label, path }) => {
            const isActive = path === '/'
              ? location.pathname === '/'
              : location.pathname.startsWith(path)
            const handleClick = path === '/'
              ? () => { navigate('/'); onDashboard?.() }
              : () => navigate(path)
            return (
              <button
                key={path}
                onClick={handleClick}
                className={`flex items-center gap-3 px-4 py-1.5 rounded-[10px] font-body font-semibold text-[16px] transition-all border ${isActive ? btnActive : btnDefault}`}
              >
                {label}
              </button>
            )
          })}

        </nav>

        {/* Droite : panier (optionnel) + compte */}
        <div className="flex items-center gap-3 flex-shrink-0">
          {onCart && (
            <button onClick={onCart} aria-label="Liste de courses" className="relative w-10 h-10 flex items-center justify-center text-ink-primary">
              <CartIcon />
              {cartCount > 0 && (
                <span className="absolute top-0.5 right-0.5 min-w-[16px] h-4 bg-brand text-ink-primary text-[14px] font-bold rounded-full flex items-center justify-center px-1">
                  {cartCount}
                </span>
              )}
            </button>
          )}
        {!authLoading && (
          isAnonymous
            ? (
              <button
                onClick={onShowPlan}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg font-body font-semibold text-[14px] border transition-all flex-shrink-0 ${btnDefault}`}
              >
                <GoogleIcon />
                Sauvegarder
              </button>
            ) : (
              <button
                onClick={onShowAccount}
                aria-label="Mon compte"
                className={`w-9 h-9 flex items-center justify-center rounded-full border transition-all flex-shrink-0 ${btnDefault}`}
              >
                <UserIcon />
              </button>
            )
        )}
        </div>

      </div>
    </header>
  )
}
